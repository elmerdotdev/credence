...
const createClient = async (req, res) => {
    const { firstname, lastname, company, email, phone, description, active, photo, user_id } = req.body

    try {
        const client = await Client.create({ firstname, lastname, company, email, phone, description, active, photo, user_id })
        res.status(200).json(client)
    } catch (error) {
